            ...
            <div className="px-5 py-4 space-y-3">
              ...
              <section className="pt-1">
                ...
                {Array.from({ length: Math.max(totalPax, 1) }).map((_, i) => {
                  const nome = (cotacao as any).passageirosNomes?.[i] || (i === 0 ? cotacao.cliente?.nome : `Passageiro ${i + 1}`) || `Passageiro ${i + 1}`;
    ...
    <section>
      <div className="bg-[oklch(0.18_0.08_255)] text-white rounded-t flex items-center justify-between px-3 py-1.5 print-bg gap-2">
        <div className="flex items-center gap-1.5 font-semibold text-xs">
          ...
          {brand && (
            <span className="ml-2 text-[10px] opacity-90 font-normal">{brand.name} · {brand.iata}</span>
          )}
        </div>
